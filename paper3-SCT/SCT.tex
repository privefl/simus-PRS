...
This is why hyper-parameters of clumping and thresholding must be picked with care when using C+T in order to maximize its predictive ability.
...
In scenario ``err'', we sample 10,000 random causal SNPs but 10\% of the GWAS effects are reported with an opposite effect in the summary statistics; it aims at assessing if methods are able to partially correct for differences in effect sizes between GWAS and the target data.
...
For details on how we define phenotypes in the UKBB, please refer to our R code. 
...
In these real data applications, we compare stdCT, maxCT and SCT. Training SCT and choosing optimal hyper-parameters for C+T use 63\%-90\% of the data reported in table \ref{tab:sumstats}. The training set can therefore contain as many as 300K individuals.
...
\[{V}_i^{(k)} \left( \text{INFO}_T,~r_c^2,~w_c,~p_T \right) = \sum_{\substack{j \in S_\text{clumping}(k,~\text{INFO}_T,~r_c^2,~w_c)}} \hat\beta_j \cdot G_{i,j} \cdot \mathbbm{1}\{p_j < p_T\}~,\] where $\hat\beta_j$ ($p_j$) are the effect sizes (p-values) estimated from the GWAS, $G_{i,j}$ is the dosage for individual $i$ and SNP $j$, and $S_\text{clumping}(k,~\text{INFO}_T,~r_c^2,~w_c)$ corresponds to first restricting to variants of chromosome $k$ with an INFO score $\ge \text{INFO}_T$ and that further result from clumping with parameters $r_c^2$ and $w_c$.
...
Effects resulting from SCT (Figure \ref{fig:neweffsimu}) are mostly comprise between the GWAS effects and 0. For the simulation with only 100 causal SNPs, resulting effects are either nearly the same as in the GWAS, or near 0 (or exactly 0).
...
Furthermore, when training size uses a large proportion of the UK Biobank data, SCT outforms maxCT for all 8 diseases considered with an additional mean absolute increase of AUC of 2.2\%, making it 3.5\% as compared to stdCT (Figure \ref{fig:AUC-real} and table \ref{tab:AUC}).
...
When training size is smaller (500 cases and 2000 controls only), SCT is not as good as when training size is large. SCT remains a competitive method expect for depression for which maxCT performs much better than SCT (Figure \ref{fig:AUC-real-small}).
...
Here, we define 4 hyper-parameters for C+T (minor allele frequency could be a fifth one). Usually, C+T scores for different p-value thresholds are derived, using some arbitrary values for the other 3 hyper-parameters.
...
Third, if reported GWAS effects are different in the data which we compute the PRS from, for example when applying SCT in a different population as the GWAS was conducted in, having SNPs with opposite effects might help adapt the GWAS effects.
...
Moreover, it remains an effective method since it can adapt to many different disease architectures by playing with all hyper-parameters.
...
As a future direction, note that we added an extra parameter in the SCT pipeline that makes possible for an user to define their own group of variants. This opens many possibilities for SCT.
