...
C+T allows to derive PRS for a sample of genotyped individuals using regression coefficients and p-values of GWAS.
...
More precisely, the SNPs are first clumped (C) so that only SNPs that are weakly correlated with each other are retained. 
Clumping looks at the most significant SNP iteratively, computes correlation between this index SNP and nearby SNPs within some genetic distance $w_c$, and removes all the nearby SNPs that are correlated with this index SNP beyond a particular value $r_c^2$. 
Thresholding (T) consists in removing variants with a p-value larger than a chosen level of significance ($p > p_T$). 
Both steps of clumping and thresholding involve statistical compromise.
...
Therefore, when applying C+T, one has to choose at least 3 hyper-parameters, which are the squared correlation threshold $r_c^2$ and the window size $w_c$ of clumping, together with the p-value threshold $p_T$.
...
The use of a liberal inclusion of variants is common, with the assumption that using more variants in the model yields better prediction, whatever the imputation accuracy of these variants. 
...
Moreover, instead of choosing one set of parameters that corresponds to the best prediction, we propose to use stacking, i.e.\ we learn an optimal linear combination of all computed C+T scores to improve prediction beyond the best prediction provided by any of these scores \cite[]{breiman1996stacked}.
...
\[{V}_i^{(k)} \left( \text{INFO}_T,~r_c^2,~w_c,~p_T \right) = \sum_{\substack{j \in S_\text{clumping}(k,~\text{INFO}_T,~r_c^2,~w_c)}} \hat\beta_j \cdot G_{i,j} \cdot \mathbbm{1}\{p_j < p_T\}~,\] where $\hat\beta_j$ ($p_j$) are the effect sizes (p-values) estimated from the GWAS, $G_{i,j}$ is the dosage for individual $i$ and SNP $j$, and the set $S_\text{clumping}(k,~\text{INFO}_T,~r_c^2,~w_c)$ corresponds to first restricting to variants of chromosome $k$ with an INFO score $\ge \text{INFO}_T$ and that further result from clumping with parameters $r_c^2$ and $w_c$.
...
In scenario ``2chr'', 100 SNPs of chromosome 1 and all SNPs of chromosome 2 are causal with half of the heritability for both chromosomes; it aims at assessing predictive performance when disease architectures are different for different chromosomes. 
In scenario ``err'', we sample 10,000 random causal SNPs but 10\% of the GWAS effects are reported with an opposite effect in the summary statistics; it aims at assessing if methods are able to partially correct for errors or mere differences in effect sizes between GWAS and the target data.
In scenario ``HLA'', 7105 causal SNPs are chosen in one long-range LD region of chromosome 6; it aims at assessing if methods can handle strong correlation between causal variants.
...
SCT also provides higher AUC than lassosum, expect when simulating all SNPs as causal (1M).

Effects resulting from SCT (Figure \ref{fig:neweffsimu}) are mostly comprised between the GWAS effects and 0. For the simulation with only 100 causal SNPs, resulting effects are either nearly the same as in the GWAS, or near 0 (or exactly 0).
...
\caption{AUC values on the test set of UKBB (mean and 95\% CI from $10^4$ bootstrap samples). Training SCT and choosing optimal hyper-parameters for C+T use 63\%-90\% of the data reported in table \ref{tab:sumstats}. See corresponding values in table \ref{tab:AUC}.}
\label{fig:AUC-real}
\end{figure}
...
Here, we define 4 hyper-parameters for C+T (minor allele frequency could be a fifth one). Usually, C+T scores for different p-value thresholds are derived, using some default values for the other 3 hyper-parameters.
In R package bigsnpr, we now provide some efficient functions to compute C+T scores for many combinations of these parameters.
Using simulated and real data, we show that choosing different values rather than default ones for these hyper-parameters has the potential to substantially improves performance of C+T, making C+T a very competitive method.
...
Combining multiple PRS is not a new idea \cite[]{krapohl2018multi,inouye2018genomic}, but we push this idea to the limit by combining 123,200 PRS.
This is both an advantage of SCT because it makes SCT more flexible than any C+T model, but it can be also a weakness as it is difficult to learn weights for all these models if training size if small.
...
Moreover, we allow C+T scores to have negative weights in the final model because we think it makes sense to do so for three reasons. First, because C+T scores are overlapping in the SNPs they use, using some negative weights allows to only weight groups of SNPs ``in the middle'' that correspond to the differences of two sets of SNPs included in C+T scores [TODO: make this sentence more understandable]. 
Second, because of LD, some SNPs might have different effects when learned jointly with others (Figures \ref{fig:neweff1M} and \ref{fig:neweffHLA}). 
Third, if reported GWAS effects are different in the data which we compute the PRS from, for example when applying SCT in a different population as the GWAS was conducted in, having SNPs with opposite effects can help adjust the effects learned during GWAS.
...
We also do not analyze many automimmune diseases as summary statistics are often outdated (2010-2011\footnote{https://www.immunobase.org/downloads/protected\_data/GWAS\_Data/}) and, because there are usually large effects on chromosome 6, methods that use individual-level data instead of summary statistics are likely to provide better predictive models \cite[]{prive2019efficient}.
...
Moreover, we focus this study on improving the C+T method. The C+T method is by far the simplest and most widely-used method for constructing polygenic risk scores based on summary statistics. 
The idea behind C+T is simple because it directly uses weights learned from GWAS; it further removes SNPs as one does when reporting hits from GWAS, i.e.\ only SNPs that pass the genome-wide threshold (p-value thresholding) and that are independent association findings (clumping) are reported. 
...
Some methods require only a small reference panel to account for LD while some methods have a phase of retraining that needs a training set with individual-level data and phenotypes.
Here, we include lassosum in the simulations since no other method has proven to be better, and because it is simple to use. 
Unfortunately, lassosum was reported to require an heavy computation time when there were too many samples or too many variants, thus we did not apply it to the full UK Biobank data. 
...
First, there can be mistakes in meta-analyses results \cite[]{greco2013meta}, mistakes in disease diagnostic \cite[]{wray2012impact,thomas2018frequency}, imprecisions in data, or disambiguating alleles \cite[]{chen2018prs}.
For example, in one simulation scenario (``err'') of this paper, we show that having an extra layer of training consisting in stacking C+T models makes it possible to partially correct for mistakes or differences in effect sizes.
...
Moreover, it remains an effective method since it can adapt to many different disease architectures by tuning all hyper-parameters.
...
Yet, to learn weights of all C+T scores may require the training size to be large enough.

[TODO: Michael doesn't understand this paragraph] As a future direction, note that we added an extra parameter in the SCT pipeline that makes possible for an user to define their own group of variants. This opens many possibilities for SCT.
For example, we could derive and stack C+T scores for two (or more) different GWAS summary statistics, e.g.\ for different ancestries or for different phenotypes. This would make SCT usable as a multivariate method. 
Another use would be to learn differentiate between two different phenotypes such as T1D and T2D.
...
\caption{AUC values on the test set of UKBB (mean and 95\% CI from $10^4$ bootstrap samples). Training SCT and choosing optimal hyper-parameters for C+T use 500 cases and 2000 controls only. See corresponding values in table \ref{tab:AUC2}.}
...
\caption{``100'': 100 random causal SNPs}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{0.7\textwidth}
\includegraphics[width=\textwidth]{new-effects-simu10K.png}
\caption{``10K'': 10,000 random causal SNPs}
\end{subfigure}
\end{figure}
...
\caption{``1M'': all 1M SNPs are causal SNPs\label{fig:neweff1M}}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{0.7\textwidth}
\includegraphics[width=\textwidth]{new-effects-simu2chr.png}
\caption{``2chr'': Causal SNPs on chromosomes 1 \& 2}
\end{subfigure}
\end{figure}
...
\caption{``err'': 10,000 random causal SNPs, but 10\% of the GWAS effects are reported with an opposite effect\label{fig:newefferr}}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{0.7\textwidth}
\includegraphics[width=\textwidth]{new-effects-simuHLA.png}
\caption{``HLA'': 7105 causal SNPs in a long-range LD region of chromosome 6\label{fig:neweffHLA}}
\end{subfigure}
...
\caption{``100'': 100 random causal SNPs\label{fig:simugridA}}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{\textwidth}
\includegraphics[width=\textwidth]{grid-simu10K.png}
\caption{``10K'': 10,000 random causal SNPs\label{fig:simugridB}}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{\textwidth}
\includegraphics[width=\textwidth]{grid-simu1M.png}
\caption{``1M'': all 1M SNPs are causal SNPs\label{fig:simugridC}}
\end{subfigure}
\end{figure}
...
\caption{``err'': 10,000 random causal SNPs, but 10\% of the GWAS effects are reported with an opposite effect}
\end{subfigure}
\\~\\~\\
\begin{subfigure}[b]{\textwidth}
\includegraphics[width=\textwidth]{grid-simuHLA.png}
\caption{``HLA'': 7105 causal SNPs in a long-range LD region of chromosome 6}
\end{subfigure}
