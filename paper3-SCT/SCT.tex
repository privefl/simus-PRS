...
We keep all variants with a GWAS p-value lower than 0.1. [TODO: EXCEPT FOR]
...
\end{itemize}
Thus, for individual $i$, chromosome $k$ and the four hyper-parameters $\text{INFO}_T$, $r_c^2$, $w_c$ and $p_T$, we compute
\[\rm{V}_i^{(k)}(\text{INFO}_T,~r_c^2,~w_c,~p_T) = \sum_{\substack{j ~\in~ \text{chromosome k} \\ \text{INFO}_j ~\geq~ \text{INFO}_T \\ j ~\in~ S_\text{clumping}(r_c^2,~w_c) \\ p_j~<~p_T}} \hat\beta_j \cdot G_{i,j}~,\] where $\hat\beta_j$ ($p_j$) are the effect sizes (p-values) estimated from the GWAS and $G_{i,j}$ is the dosage for individual $i$ and SNP $j$.
