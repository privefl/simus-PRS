...
\begin{table}[h]
\caption{Choice of C+T parameters based on the maximum AUC in the training set. Hyper-parameters of C+T are the squared correlation threshold $r_c^2$ and the window size $w_c$ of clumping, the p-value threshold $p_T$ and the threshold on the quality of imputation $\text{INFO}_T$. Choosing optimal hyper-parameters for C+T use 63\%-90\% of the individuals reported in table \ref{tab:sumstats}. Resulting predictions of maxCT in the test set are reported in figure \ref{fig:AUC-real}.\label{tab:param}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|c|c|c|c|}
  \hline
...
One can of course also optimize lassosum and LDpred, which could result in better relative performance. For instance, recent work by \cite{marquez2018modeling} suggests that excluding long-range LD regions when applying LDpred can improve its predictive performance. A possible upcoming feature of LDpred is to allocate heritability estimation differently along the genome. Also, LDpred has been reported to not adequately adjust for the LD structure when sample size of summary statistics is large, and that using alternative methods such as PRS-CS may solve this issue \cite[]{ge2019polygenic}.
However, we did not explore non-standard parameter settings for applications of LDpred and lassosum, or extensions of LDpred.
