...
(C+T) is a widely used method to derive polygenic scores. When using C+T, it is common to
...
efficient way to derive thousands of different C+T polygenic scores corresponding to a grid over four hyper-parameters. For example, it takes a few hours to derive 123,200 different C+T scores for 300K individuals and 1M variants on a single node with 16 cores. 
...
Note that the set used to choose optimal hyper-parameters is commonly called ``validation set'', yet, as it refers to the same set used for learning weights of stacking in SCT, we uniformly refer to this set as ``training set'' here. 
...
\begin{tabular}{|l|c|c|c|c|}
  \hline
Trait & UKBB size -- training (\%) & GWAS size & GWAS \#variants & GWAS citation \\
  \hline
Breast cancer (BRCA) & ~~11,578 / 158,391 -- 150,000 (88) & 137,045 / 119,078 & 11,792,542 & \cite{michailidou2017association} \\
Rheumatoid arthritis (RA) & ~~~~~5615 / 226,327 -- 200,000 (86) & ~~29,880 / ~~73,758 & ~~9,739,303 & \cite{okada2014genetics} \\
Type 1 diabetes (T1D) & ~~~~~~~771 / 314,547 -- 200,000 (63) & ~~~~~5913 / ~~~~~8828  & ~~8,996,866 & \cite{censin2017childhood} \\
Type 2 diabetes (T2D) & ~~14,176 / 314,547 -- 250,000 (76) & ~~26,676 / 132,532 & 12,056,346 & \cite{scott2017expanded} \\
Prostate cancer (PRCA) & ~~~~~6643 / 141,321 -- 120,000 (81) & ~~79,148 / ~~61,106 & 20,370,946 & \cite{schumacher2018association} \\
Depression (MDD) & ~~22,287 / 255,317 -- 250,000 (90) & ~~59,851 / 113,154 & 13,554,550 & \cite{wray2018genome} \\
Coronary artery disease (CAD) & ~~12,263 / 225,927 -- 200,000 (84) & ~~60,801 / 123,504 & ~~9,455,778 & \cite{nikpay2015comprehensive} \\
Asthma & ~~43,787 / 261,985 -- 250,000 (82) & ~~19,954 / 107,715 & ~~2,001,280 & \cite{demenais2018multiancestry} \\
  \hline
\end{tabular}
\end{table}
...
Results are similar when using less well imputed variants in the simulations. Globally in these simulations, including a broad range of imputed variants with INFO score as low as 0.3 often maximizes prediction (Figures \ref{fig:simu1grid} and \ref{fig:simu2grid}).
...
In terms of AUC, maxCT outperfoms stdCT for all 8 diseases considered with a mean absolute increase of 1.3\% (Figures \ref{fig:AUC-real} and \ref{fig:AUC-real-small}).
...
\caption{AUC values on the test set of UKBB (mean and 95\% CI from $10^4$ bootstrap samples). Training SCT and choosing optimal hyper-parameters for C+T and lassosum use 63\%-90\% of the individuals reported in table \ref{tab:sumstats}. See corresponding values in table \ref{tab:AUC}.}
\label{fig:AUC-real}
\end{figure}

\begin{table}[h]
\caption{Choice of C+T parameters based on the maximum AUC in the training set. Choosing optimal hyper-parameters for C+T use 63\%-90\% of the individuals reported in table \ref{tab:sumstats}. Resulting predictions of maxCT in the test set are reported in figure \ref{fig:AUC-real}.\label{tab:param}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|c|c|c|c|}
  \hline
...
When training size is smaller (500 cases and 2000 controls only instead of 120K-250K individuals), SCT is not as good as when training size is large, yet SCT remains a competitive method expect for depression for which maxCT performs much better than all other methods (Figure \ref{fig:AUC-real-small}).
...
However, even though lassosum can provide more accurate prediction for T2D and CAD, it can also perform very poorly for other diseases such as BRCA, PRCA and MDD (Figures \ref{fig:AUC-real} and \ref{fig:AUC-real-small}). 
LDpred performs much better than the other methods for predicting MDD, yet maxCT and SCT perform equally or better than LDpred for the 7 other traits (Figure \ref{fig:AUC-real-small}).
...
One could also consider other variants of C+T such as choosing a different set of hyper-parameters for each chromosome separately, which would make a lot of sense e.g. in the ``2chr'' simulation scenario.
...
Finally, note that there is an extra parameter in the SCT pipeline that makes possible for an user to define their own groups of variants. This allows to refine the grid of computed C+T scores and opens many possibilities for SCT.
...
For MDD, we tested to add a threshold on MAF as a fifth hyper-parameter of C+T. This improved AUC of maxCT from 59.2\% to 60.7\% (Figure \ref{fig:grid-MAF}). For maxCT, the optimal threshold on MAF among $\{10^{-6}, 10^{-5}, ..., 10^{-1}\}$ was 0.01 (Figure \ref{fig:grid-MAF}). Note that this threshold on MAF now makes maxCT more predictive than LDpred that internally uses a MAF threshold of 0.01 by default.
...
Moreover, as we show here, when C+T is optimized using a large grid of hyper-parameters, it becomes a very competitive method since it can adapt to many different disease architectures by tuning all hyper-parameters.
...
\begin{table}[!htpb]
\caption{Results of the 6 simulation scenarios with well imputed variants: (100) 100 random causal variants; (10K) 10,000 random causal variants; (1M) all 1M variants are causal variants; (2chr) 100 variants of chromosome 1 are causal and all variants of chromosome 2, with half of the heritability for both chromosomes; (err) 10,000 random causal variants, but 10\% of the GWAS effects are reported with an opposite effect; (HLA) 7105 causal variants in a long-range LD region of chromosome 6. Mean and 95\% CI of $10^4$ non-parametric bootstrap replicates of the mean AUC of 10 simulations for each scenario. These results are plotted in figure \ref{fig:AUC-simu1}.\label{tab:AUC-simu1}}
\vspace*{0.5em}
\centering
...
\begin{table}[!htpb]
\caption{AUC values on the test set of UKBB (mean [95\% CI] from $10^4$ bootstrap samples) and the number of variants used in the final model. Training SCT and choosing optimal hyper-parameters for C+T and lassosum use 63\%-90\% of the individuals reported in table \ref{tab:sumstats}. These results are plotted in figure \ref{fig:AUC-real}.\label{tab:AUC}}
\vspace*{0.5em}
\centering
\begin{tabular}{|l|c|c|c|c|}
  \hline
...
\caption{Mean and 95\% CI of $10^4$ non-parametric bootstrap replicates of the mean AUC of 6 random splits to define the training set. Training SCT and choosing optimal hyper-parameters for C+T, LDpred and lassosum use 500 cases and 2000 controls only. See corresponding values in table \ref{tab:AUC2}.}
