...
\abstract{
Polygenic prediction has the potential to contribute to precision medicine. 
Clumping and Thresholding (C+T) is a widely used method to derive polygenic scores.
When using C+T, people usually test several p-value thresholds to maximize predictive ability of derived polygenic scores.
...
Using simulated and real data, we show that choosing different values rather than default ones for these hyper-parameters can substantially improve the performance of C+T, making C+T a very competitive method as good as more sophisticated methods such as lassosum.
...
Combining multiple PRS is not a new idea \cite[]{krapohl2018multi,inouye2018genomic,maier2018improving}, but we push this idea to the limit by combining 123,200 polygenic scores.
This is an advantage of SCT because it makes SCT more flexible than any C+T model, but it of course also requires a larger training dataset with individual-level genotypes and phenotypes to learn the weights.
...
Yet, learning weights of all C+T scores may require the training size to be large enough.
