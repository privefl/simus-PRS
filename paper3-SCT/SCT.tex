...
\abstract{
Polygenic prediction has the potential to contribute to precision medicine. Clumping and Thresh-
olding (C+T) is a widely used method to derive polygenic scores. When using C+T, several p-value thresholds are tested to maximize predictive ability of the derived polygenic scores. Along
...
noted when predicting depression status, from an AUC of 0.557 (95\% CI: [0.544-0.569]) when tun-
ing only the p-value threshold to an AUC of 0.592 (95\% CI: [0.580-0.604]) when tuning all
...
combination of all C+T scores by using an efficient penalized regression. We apply SCT to 8 differ-
ent case-control diseases in the UK biobank data and find that SCT substantially improves prediction
...
~\\ \textit{Declaration of Interests:} the authors declare no competing interests.
