...
bed <- snp_writeBed(ukbb, bedfile = "data/UKBB_BRCA_lassosum.bed",
                    ind.row = ind.train)
library(doParallel)
...
system.time(
  out <- lassosum.pipeline(
    ...
    A1 = ukbb$map$allele1,
    test.bfile = "data/UKBB_BRCA_lassosum",
    LDblocks = "EUR.hg19",
    cluster = cl,
    sample = 20e3,
...
length(ind <- which(v$best.beta != 0))  # 322,003
pred_lassosum <- big_prodVec(G, v$best.beta[ind], ind.row = ind.test, ind.col = ind)
AUCBoot(pred_lassosum, y.sub[ind.test]) # 57.9 [56.3-59.5] / 57.8 [57.3-58.4]
