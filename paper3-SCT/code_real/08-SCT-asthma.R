...
bed <- snp_writeBed(ukbb, bedfile = "data/UKBB_asthma_lassosum.bed",
                    ind.row = ind.train)
library(doParallel)
registerDoParallel(cl <- makeCluster(NCORES))
system.time(
  out <- lassosum.pipeline(
    cor = p2cor(p = 10^-lpval, n = 19954 + 107715, sign = beta),
    snp = ukbb$map$marker.ID,
    A1 = ukbb$map$allele1,
    test.bfile = "data/UKBB_asthma_lassosum",
    LDblocks = "EUR.hg19",
    cluster = cl,
    sample = 20e3,
...
length(ind <- which(v$best.beta != 0))  # 75,965
pred_lassosum <- big_prodVec(G, v$best.beta[ind], ind.row = ind.test, ind.col = ind)
AUCBoot(pred_lassosum, y.sub[ind.test])  # 58.7 [58.1-59.4] / 57.0 [56.7-57.3]
